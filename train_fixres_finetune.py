...
from utils_train_test import validate, validate_corrupted, validate_tin_corrupted, AverageMeter, accuracy
...
parser = argparse.ArgumentParser(description='Blur curriculum training, discrete')
...
def main():
    ...
    # Load data
    train_loader, val_loader, numberofclass, normalize, lam = load_dataset(args.dataset, args.data_dir, args.batch_size, args.workers)

    # Define model architecture
    print("=> creating model '{}'".format(args.net_type))
    assert args.net_type == 'preactresnet18'
    ...
    out_dir = os.path.join(f'runs_2023/{args.dataset}/{args.net_type}/clewr_discrete/sigmas_{args.sigmas}_epochs_{args.epochs}_{args.expname}/')
    ...
    if args.dataset.startswith('cifar'):
        validate_corrupted(model, criterion, args.corrupted_data_dir, csv_filepath, val_loader, normalize, args.batch_size, args.workers)
...
def train(train_loader, model, criterion, optimizer, epoch, sigma_arr, weightage):
    ...
    # we don't want to train all BN layers, only the last one
    model.eval()
    # model.module.bn.train() # note: this is specific to preactresnet18
